...
const ShopWithSidebar = () => {
  ...
  const [priceRange, setPriceRange] = useState({from:0, to:999})
  const [showStyle, setShowStyle] = useState(0);

  useEffect(() => {
    if(category.length == 0){
      setShowData(shopData.filter(product => product.price >= priceRange.from && product.price <= priceRange.to));
    }
    else {
      setShowData(shopData.filter(product => product.price >= priceRange.from && product.price <= priceRange.to && category.includes(product.category)));
    }
    console.log(priceRange, category)
  }, [shopData, priceRange, category])
  ...
  useEffect(() => {
    ...
    setShowData(prev => {
      const sorted = [...prev]; // make a copy first
      if (showStyle == 0) {
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    <div className="flex items-center justify-between">
                      <p>Filters:</p>
                      <button className="text-blue" onClick={() => {setCategories([]), setPriceRange({from:0, to:999})}}>Clean All</button>
                    </div>
                  </div>

                  {/* <!-- category box --> */}
                  <CategoryDropdown categories={categories} setCategories={setCategories} selectedCategories={category}/>
                  {/* // <!-- price range box --> */}
            ...
            <div className="xl:max-w-[870px] w-full">
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <CustomSelect options={options} setShowStyle={setShowStyle} />

                    <p>
                      Showing <span className="text-dark">{`${showData.length} of ${shopData.length}`}</span>{" "}
                      Products
                    </p>
                  </div>
              ...
              >
                {showData.slice((currentPage-1)*9, currentPage*9).map((item, key) =>
                  productStyle === "grid" ? (
              ...
              {
                showData.length > 9 && <Pagination currentPage={currentPage} totalPages={Math.ceil(showData.length/9)} setCurrentPage={setCurrentPage} />
              }
